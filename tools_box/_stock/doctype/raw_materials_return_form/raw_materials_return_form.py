...
class RawMaterialsReturnForm(Document):
    ...
    def on_change(self):
        ...
        if self.workflow_state == "Received":
            ...
            stock_entry.title = "Raw Material Return from ".format(self.production_order)
            stock_entry.from_warehouse = get_wip(self.production_order)

            stock_entry.work_order = self.production_order
            ...
            for index, value in enumerate(self.items):
                ...
                if index == 0:
                    stock_entry.to_warehouse = cur_item[0].default_warehouse
            ...
            stock_entry.insert()
            stock_entry.submit()
            update_me(self.name, stock_entry.name, self.production_order)
...
def update_me(me, se, po):
    try:
        ...
        frappe.db.sql(
            "UPDATE `tabWork Order` SET has_raw_returned = 1, raw_material_return = '{0}' WHERE name = '{1}'"
                .format(me, po))
...
def get_wip(name):
    _ = frappe.db.sql("SELECT wip_warehouse FROM `tabWork Order` WHERE name = '%s'" % name , as_dict=1)
    if len(_):
...
def update_production_order(name,status):
    _ = frappe.db.sql("SELECT qty, produced_qty FROM `tabWork Order` WHERE name = '%s' AND status != 'Stopped' "
                      % name, as_dict=1)
    if len(_) and (_[0].qty < _[0].produced_qty):
        ...
        frappe.db.sql("UPDATE `tabWork Order` SET status = '%s', docstatus =1 WHERE name = '%s' AND status != 'Stopped'"
              % (status, name))
