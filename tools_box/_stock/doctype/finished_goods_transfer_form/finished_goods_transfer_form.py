...
class FinishedGoodsTransferForm(Document):
    ...
    def on_change(self):
        ...
        if self.workflow_state == "Received":
            ...
            stock_entry.title = "Excess Material Receipt from {0}".format(self.production_order)
...
def update_me(me, se, po):
    ...
    frappe.db.sql(
        "UPDATE `tabWork Order` SET has_excess = 1, excess_ref = '{0}' WHERE name = '{1}'"
            .format(me, po))
...
def get_producted_items(production_order=None):
    if production_order != None:
        prod = frappe.get_list(doctype="Work Order", filters={
