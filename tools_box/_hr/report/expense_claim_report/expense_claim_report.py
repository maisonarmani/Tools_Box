...
def execute(filters=None):
    ...
    columns = ["Date:Date:100", "Employee:Text:150", "Expense Claim:Link/Expense Claim:150", "Expense Claim Type:Link/Expense Claim Type:100",
               "Description:Text:200", "Sanctioned Amount:Currency:120", "Total Amount Reimbursed:Currency:120",
               "Cost Center:Link/Cost Center:130","Workflow Status:Text:100","Status:Text:100", "Approver:Link/User:200"]
    ...
    data = frappe.db.sql(
        "SELECT d.expense_date,p.employee_name,p.name,d.expense_type,d.description,d.sanctioned_amount,p.total_amount_reimbursed, p.cost_center,p.workflow_state,p.status,p.exp_approver {0} FROM "
