...
cur_frm.set_query("approver", function () {
    return {
        query: "tools_box.tools_box.doctype.work_card.work_card.get_work_card_approver"
    };
});
frappe.ui.form.on('Work Card', {
    ...
    refresh: function (frm) {
        if (cur_frm.doc.asset_category != "Plant and Machinery")
            cur_frm.toggle_reqd("ticket_number", true)
        frm.set_query("work_completion_verified_by", get_employees);
        frm.set_query("requested_by", get_employees);
        if (cur_frm.doc.status == 'Not Completed') {
            cur_frm.add_custom_button('Make Purchase Order', function () {
                frappe.model.open_mapped_doc({
                    method: "tools_box.tools_box.doctype.work_card.work_card.make_purchase_order",
                    frm: cur_frm
                });
            });
        }
        ...
        if ((cur_frm.doc.equipment_maintenance_log || !cur_frm.doc.vendor) &&
            ...
            cur_frm.add_custom_button('Make Expense Claim', function () {
                frappe.model.open_mapped_doc({
                    method: "tools_box.tools_box.doctype.work_card.work_card.make_expense_claim",
                    frm: cur_frm
                });
            });
            cur_frm.add_custom_button('Make Employee Advance', function () {
                frappe.model.open_mapped_doc({
                    method: "tools_box.tools_box.doctype.work_card.work_card.make_employee_advance",
                    frm: cur_frm
                });
            });
        }
        ...
        if (!frappe.user.has_role(['Helpdesk Admin'])) {
            ...
            cur_frm.set_df_property('work_completion_date', 'read_only', 0);
            cur_frm.set_df_property('work_completion_verified_by', 'read_only', 0);
    ...
    ticket_number: function (frm, cdt, cdn) {
        var work_card = frappe.model.get_doc(cdt, cdn);
        if (work_card.ticket_number) {
            frappe.call({
                method: "tools_box.tools_box.doctype.work_card.work_card.get_requested_by",
                args: {
                    ticket_type:work_card.ticket_type,
                    ticket_number: work_card.ticket_number
                },
                callback: function (r) {
            ...
            frappe.call({
                method: "tools_box.tools_box.doctype.work_card.work_card.get_employee_name",
                args: {
                    ticket_type:work_card.ticket_type,
                    ticket_number: work_card.ticket_number
                },
    ...
    work_advance: function (frm, cdt, cdn) {
        frappe.model.set_value(cdt, cdn, "balance_due_upon_work_completion", frappe.model.get_value(cdt, cdn, "work_card_total") - frappe.model.get_value(cdt, cdn, "work_advance"));
    },

    labour_fees: function (frm, cdt, cdn) {
        frappe.model.set_value(cdt, cdn, "work_card_total", frappe.model.get_value(cdt, cdn, "materials_total") + frappe.model.get_value(cdt, cdn, "labour_fees") + frappe.model.get_value(cdt, cdn, "transport_fare"));
        frappe.model.set_value(cdt, cdn, "balance_due_upon_work_completion", frappe.model.get_value(cdt, cdn, "work_card_total") - frappe.model.get_value(cdt, cdn, "work_advance"));
    },
...
frappe.ui.form.on('Work Card Material Detail', {
...
function recalculate_total(cdt,cdn){
        var _total = 0;
        cur_frm.doc.work_card_material_detail.forEach(function(v){ _total += v.total; });
        frappe.model.set_value(cdt, cdn, "work_card_total", frappe.model.get_value(cdt, cdn, "work_card_total") +  _total);
        frappe.model.set_value(cdt, cdn, "balance_due_upon_work_completion", frappe.model.get_value(cdt, cdn, "work_card_total") - frappe.model.get_value(cdt, cdn, "work_advance"));
}
